...
class CatalogController {
	...
	@RequestMapping("/sufu")
	public String sufud(){
	    return "redirect:/sufu/default";
	}
	
	@RequestMapping(value ={"/sufu/{genre}"}, method = RequestMethod.GET)
	public String sufu(ModelMap modelMap, @PathVariable("genre") String genre){
		HashMap map = new HashMap();
		List list = new ArrayList();
		for(Disc disc : videoCatalog.findAll()){
			String discGenre = disc.getGenre().replace("/", "-");
			if(!map.containsKey(discGenre)){
				map.put(discGenre, new ArrayList());
            }
            list = (List)map.get(discGenre);
            list.add(disc);
		}

		modelMap.addAttribute("filter", map.get(genre) );
		modelMap.addAttribute("genres", map.keySet());
			return "sufu";
	}
	...
	@RequestMapping(value={"/suche{sucheingabe2}"}, method =RequestMethod.GET)
	public String suche(ModelMap modelMap, @RequestParam("Sucheingabe") String sucheingabe, @PathVariable("sucheingabe2") String sucheingabe2 ){
		 Map<String, List> m= new HashMap<String,List>();
		 Disc find_disc = null;
		 List<Disc> l= new ArrayList<Disc>();
		
		for(Disc disc : videoCatalog.findAll()){
			String name = disc.getName();
			if(name.matches(sucheingabe)){
				find_disc= disc;
				l.add(find_disc);
				m.put(find_disc.getName(),l);
			}
			
		} 
		
		modelMap.addAttribute("find_disc",m.get(sucheingabe));
		return "suche";

...
class CartController {
	...
	public String cancel(@ModelAttribute Cart cart, @LoggedIn Optional<UserAccount> userAccount) {
	...
	/*@RequestMapping(value="/teilbestellung_entfernen", method = RequestMethod.GET)
	public String teilbestellung_entfernen (@RequestParam("entfernen") int amount, @ModelAttribute Cart cart, @LoggedIn Optional<UserAccount> userAccount){

		return userAccount.map(account -> {
			cart.removeItem(disc.);
			cart.addOrUpdateItem(disc, Quantity.of(amount));
			return "redirect:/cart";
		}).orElse("redirect:/");
	}

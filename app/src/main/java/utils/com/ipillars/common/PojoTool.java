...
public class PojoTool {
	...
	/**
	 * Method to make a clone of an object. It doesn't care if it is serializable or clonable. It goes through the columns (columns) in the object and use the getter method to get the value and use the setter method to set the values. If it is accessible (public) then it will assign it directly
	 *
	 * @param objectToCopy
	 *            - Object to make a (copy) clone of
	...
	/**
	 * Get all the columns in the class including the super class. declaredFields returns only the locally declared columns. This method recurse through the hierarchy of super classes and returns all the columns.
	 * 
	 * @param aClass
	...
	public static boolean isSimpleObject(Class dataTypeClass) {
		...
		if ((dataTypeClass == String.class) || (dataTypeClass == Boolean.class || dataTypeClass == Boolean.TYPE) || (dataTypeClass == Byte.class || dataTypeClass == Byte.TYPE) || (dataTypeClass == Short.class || dataTypeClass == Short.TYPE) || (dataTypeClass == Integer.class || dataTypeClass == Integer.TYPE) || (dataTypeClass == Long.class || dataTypeClass == Long.TYPE) || (dataTypeClass == Float.class || dataTypeClass == Float.TYPE) || (dataTypeClass == Double.class || dataTypeClass == Double.TYPE) || (dataTypeClass == Date.class)) {
			retval = true;
		}
		return retval;
	}
